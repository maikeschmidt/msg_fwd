...
filenames = {
 ...
 % 'geometries_anatom_full_inhomo', ... %this crashed - check bone
 % 'geometries_canon_cervical_homo', ... %this crashed - check bone
...
% === intersection info from TetGen ===
% seg = [3904,19];
% facet = [15391,5495,15144];
% pt_int = [0.0146764,0.135196,-0.00694468];
% 
% v = merged_mesh.p;
% f = merged_mesh.e;
% 
% figure('Color','w');
% hold on; axis equal;
% title('TetGen intersection visualization (edges highlighted)');
% xlabel('X'); ylabel('Y'); zlabel('Z');
% view(3); grid on;
% 
% % --- 1. Plot full mesh as faint wireframe ---
...
%       'EdgeAlpha', 0.3);
% 
% % --- 2. Highlight the intersecting facet ---
% patch('Faces', facet, 'Vertices', v, ...
%       'FaceColor', [1 0.2 0.2], ...
%       'EdgeColor', 'k', ...
%       'LineWidth', 1.5, ...
%       'FaceAlpha', 0.9);
% 
% % --- 3. Highlight the intersecting segment ---
% plot3(v(seg,1), v(seg,2), v(seg,3), '-o', ...
%       'Color', [0 0 0], ...
%       'LineWidth', 3, ...
%       'MarkerFaceColor', 'y', ...
%       'MarkerEdgeColor', 'k', ...
...
% plot3(pt_int(1), pt_int(2), pt_int(3), 'p', ...
%       'MarkerSize', 14, ...
%       'MarkerFaceColor', 'm', ...
%       'MarkerEdgeColor', 'k', ...
%       'LineWidth', 1.5);
% 
% legend({'Mesh edges','Intersecting facet','Intersecting segment','Intersection point'}, ...
