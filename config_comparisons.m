...
% --- mesh resolution -----------------------------------------------------
% In the hierarchy table this appears as "Mesh refinement": coarsest against
% finest volume mesh, i.e. the largest change resolution alone produces.
'conv_within_bem', 'Mesh refinement, within BEM', 'supp', 'convergence', 'within_bem', ...
    '', '', 'back'
'conv_within_fem', 'Mesh refinement, within FEM', 'supp', 'convergence', 'within_fem', ...
    '', '', 'back'
'conv_cross_solver', 'Mesh refinement, BEM vs FEM', 'supp', 'convergence', 'cross_solver', ...
    '', '', 'back'

% --- source space refinement ---------------------------------------------
% Refining the CORD compartment only, with the global tetrahedron bound
% held fixed. Reported in the hierarchy table as "Source space refinement",
% kept separate from mesh refinement because it asks whether the
% discretisation near the sources matters once the rest of the volume is
% settled.
'cord_within_fem', 'Source space refinement, within FEM', 'supp', 'cord_refine', 'within_fem', ...
    '', '', 'back'
'cord_vs_fem', 'Source space refinement vs FEM realistic', 'supp', 'cord_refine', 'within_fem', ...
    mk_key('fem','realistic','back'), '', 'back'
'cord_vs_bem', 'Source space refinement vs BEM realistic', 'supp', 'cord_refine', 'cross_solver', ...
    mk_key('bem','realistic','back'), '', 'back'
};
...
% Groups are named by what they show, and that name becomes the output
% subfolder, e.g. results/absmax/tor_comp_bem/.
%
%   tor_comp    toroidal check: homogeneous vs inhomogeneous bone. Asks
%               whether filling the vertebral torus changes the field.
%   bone_comp   the bone model progression: continuous -> toroidal ->
%               realistic, i.e. how much anatomical detail is needed.
%   bone_both   all four variants, both solvers, for every-against-every
%               heatmaps and summary tables.

grp = @(m, vars) cellfun(@(v) mk_key(m, v, core_array), vars, 'uni', 0);

CMP_GROUPS = struct( ...
    'id', { ...
        'tor_comp_bem',  'tor_comp_fem', ...
        'bone_comp_bem', 'bone_comp_fem', ...
        'bone_both'}, ...
    'label', { ...
        'BEM: homogeneous vs toroidal', 'FEM: homogeneous vs toroidal', ...
        'BEM bone models',              'FEM bone models', ...
        'All bone models, both solvers'}, ...
    'dest', {'supp', 'supp', 'main', 'main', 'supp'}, ...
    'members', { ...
        grp('bem', {'homo','inhomo'}), ...
        grp('fem', {'homo','inhomo'}), ...
        grp('bem', {'cont','inhomo','realistic'}), ...
        grp('fem', {'cont','inhomo','realistic'}), ...
        [grp('bem', bone_variants), grp('fem', bone_variants)] }, ...
    'labels', { ...
...
% Groups drawn by the curve/overlay scripts. Each writes to its own
% subfolder named by group id.
absmax_groups = {'tor_comp_bem','tor_comp_fem','bone_comp_bem','bone_comp_fem'};   % SET THIS

% PER-SOURCE SELECTIONS
% plot_per_source_cc_re runs once per entry, each into its own subfolder of
% results/per_source_cc_re. Splitting them keeps each figure readable —
% every og pair on one axis would be ten overlapping lines.
%   {folder name, cmp_select filters}
per_source_sets = { ...
    'tor_comp',    {'id', {'bem_homo_vs_inhomo','fem_homo_vs_inhomo'}}; ...
    'bone_comp',   {'id', {'bem_realistic_vs_cont','bem_realistic_vs_inhomo', ...
                           'fem_realistic_vs_cont','fem_realistic_vs_inhomo'}}; ...
    'solver',      {'dataset','og','kind','cross_solver'}; ...
    'all_og',      {'dataset','og'}; ...
};
